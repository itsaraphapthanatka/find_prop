...
> เดิมชื่อ "Find Prop" แล้วเปลี่ยนเป็น "FinProp" แล้วเปลี่ยนอีกครั้งเป็นชื่อสุดท้าย **"HOP"**
...
- ฟอร์มเพิ่ม/แก้ไขแบบ 5 ขั้น (HOP Form — ดู `docs/hop-form-spec.md`) ครบ 94 ฟิลด์,
  ชุดคำถามเปลี่ยนตามหมวด/ประเภททรัพย์ + ผูกกับเช่า-ขาย, บันทึกร่างอัตโนมัติระหว่างกรอก,
  dropdown "เลือกหรือพิมพ์เพิ่ม" + multi-select, อัปโหลดรูปหลายรูปขึ้น Supabase Storage,
...
### 5. นัดติดตาม (FollowUpPage + แผงในหน้ารายละเอียดทรัพย์)
- ตาราง `follow_ups` (`supabase/follow-ups.sql`, ผล = `followups-result.sql`, กั้นแพ็กเกจ = `followups-pro-gate.sql`)
- แท็บต้องทำ/ที่จะถึง/เสร็จแล้ว · ปิดนัดพร้อมผล → จบเรื่อง หรือ ตามต่อ (สร้างนัดรอบใหม่) เก็บประวัติทุกรอบ
- push แจ้งเตือน 07:00 ของวันครบกำหนด (`api/push-cron.js`) · ปิดงานทรัพย์ = `properties.deal_status`
  (`property-deal-status.sql`) ปิดนัดค้างทั้งหมด + AI ไม่เสนอทรัพย์ที่ปิดงานแล้ว
- ฟีเจอร์ Pro (route `/followups` กั้นด้วย `usePlanAccess().followUps`)

### 6. ชอร์ตลิสต์เปรียบเทียบ (ComparePage) — ใหม่
- เลือกทรัพย์ 2–4 รายการเทียบสเปกเป็นตาราง, AI เขียนบทวิเคราะห์จุดเด่น/ข้อควรพิจารณา/คำแนะนำ
- พิมพ์/บันทึก PDF หัวกระดาษ HOP ส่งลูกค้าได้ทันที

### 7. นำเข้าข้อมูล Excel/CSV (ImportPage) — ใหม่
...
### 8. Dashboard สรุปภาพรวม (DashboardPage) — ใหม่
...
### 9. ผู้ช่วย AI แชทลอยทุกหน้า (Assistant) — ใหม่
...
### 10. Design System ("Kraken") + PWA + Responsive
...
### 11. ระบบทีม/องค์กร/สิทธิ์ (คงจากรอบก่อน + เพิ่มเติม)
...
### 12. Landing Page ขายแพลตฟอร์ม — ใหม่
- ผู้เยี่ยมชมทั่วไป (ยังไม่ล็อกอิน, เข้าทางเว็บ) เห็นหน้าขาย HOP เป็น SaaS:
...
### 13. แอปมือถือ Android (Capacitor) — ใหม่
