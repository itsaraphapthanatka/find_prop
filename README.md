...
- 🎤 **บันทึกด้วยเสียง** — กดพูดหน้างานแล้วเล่ารายละเอียดทรัพย์รวดเดียว **AI** แกะเป็นฟิลด์
  กรอกฟอร์มให้อัตโนมัติ (รวมเลขที่พูดเป็นคำไทย เช่น "แปดหมื่นห้า" → 85,000)
  และพูดใส่ requirement ในหน้าแผนเยี่ยมชม/เปรียบเทียบได้ — ถอดเสียงด้วย Web Speech API
  ฟรีในเบราว์เซอร์ (Chrome/Android แม่นสุด, เบราว์เซอร์ที่ไม่รองรับจะซ่อนปุ่มพูดให้เอง)
- 📈 **สรุปภาพรวม (Dashboard)** — ตัวเลขพอร์ต (มูลค่าเช่า/ขายรวม, ทรัพย์ใหม่+sparkline),
  นัดเยี่ยมชมที่ใกล้ถึงพร้อมปุ่มนำทาง, สัดส่วนประเภท/เช่า-ขาย (จิ้มไปหน้ารายการพร้อมกรอง),
  ทำเลยอดนิยม, ค่าเช่าเฉลี่ย/ตร.ม.ต่อประเภท + ทรัพย์ที่ถูก/แพงกว่าค่าเฉลี่ยกลุ่ม,
  สุขภาพข้อมูล (ไม่มีรูป/พิกัด/ราคา/เบอร์/ค้างเก่า) และ **AI วิเคราะห์พอร์ตรายวัน**
  (จุดแข็ง, demand ลูกค้าที่ยังไม่มีของ, ทรัพย์ควรเร่งนำเสนอ)
- ✨ **ผู้ช่วย AI** — ปุ่มลอยเปิดแชทได้จากทุกหน้า ถามข้อมูลทรัพย์/แผนเยี่ยมชมของทีมเอง
  ("โกดังแถวบางพลีงบไม่เกินแสนมีไหม") ได้คำตอบพร้อมการ์ดทรัพย์กดดูต่อได้
  และสั่งงานผ่านแชท (เพิ่ม/ถอดทรัพย์จากรูท, สร้างแผน, เปิดหน้าเปรียบเทียบ)
  ทุกคำสั่งมีปุ่มยืนยันก่อนทำจริง · พูดถามด้วยไมค์ + ให้อ่านคำตอบออกเสียงได้ (โหมดขับรถ)
...
2. เปิด **SQL Editor** ในโปรเจกต์ → รันตามลำดับ:
   ...
   → *(ทางเลือก)* [`supabase/demo-org.sql`](supabase/demo-org.sql) (องค์กรเดโม "Demo Estate" + ทรัพย์ตัวอย่าง 10 รายการ — ไว้ทดสอบมุมมอง super/เดโมขายแพลตฟอร์ม, รันซ้ำได้)
3. ตั้งค่า Auth:
