...
/**
 * ค่าที่อาจ "ถูกปิดตามสิทธิ์" (masked) ต้องไม่ขึ้นว่า "ไม่ได้ระบุ" — ไม่งั้นอ่านผิดเป็น
 * "ไม่ได้กรอกข้อมูล" ทั้งที่กรอกไว้แต่บทบาทนี้ไม่มีสิทธิ์เห็น (ดู supabase/roles.sql)
 * masked = ซ่อนแถวไปเลย เพราะมีแถบ 🔒 อธิบายรวมอยู่ด้านบนแล้ว
 */
const orNotSpecified = (value: React.ReactNode, masked?: boolean | null): React.ReactNode =>
  masked ? null : (value || NotSpecified)
    ...
    <>
      ...
      <aside className="detail-pane">
        ...
        <div className="detail-body">
          ...
          <Field label={LABELS.lessor_name} value={orNotSpecified(p.lessor_name, p.contact_masked)} />
          <Field
            label={LABELS.phone}
            value={orNotSpecified(p.phone && (
              <>
                {p.phone}{' '}
