...
const GALLERY = [
  ...
  { src: '/app-compare.jpg', title: 'เอกสารเปรียบเทียบ', sub: 'เทียบสเปก + ส่งลิงก์ให้ลูกค้า' },
...
const FEATURES = [
  {
    ...
    desc: 'สมัครด้วยอีเมลหรือ Google แล้วใช้ได้เลย ไม่ต้องรอทีมงานเปิดบัญชี — {trial} ไม่ต้องผูกบัตรเครดิต',
    icon: <IconUser size={22} />,
  },
  {
    title: 'ฐานข้อมูลทรัพย์ + แผนที่ดาวเทียม',
    desc: 'เก็บทรัพย์ทุกประเภทไว้ที่เดียว พร้อมรูป เอกสาร และหมุดบนแผนที่ดาวเทียม ฟอร์มถามเฉพาะข้อมูลของทรัพย์ประเภทนั้น ค้นหาเจอในไม่กี่วินาที',
    icon: <IconMap size={22} />,
  },
  {
    title: 'พูดปุ๊บ ถ่ายรูปปั๊บ ได้ข้อมูล',
    desc: 'ยืนอยู่หน้างานก็กดอัดเสียง เล่ารายละเอียดทรัพย์ แล้ว AI พิมพ์ลงฟอร์มให้เอง ถ่ายรูปแนบได้ทันที',
    icon: <IconMic size={22} />,
  },
  {
    title: 'ทำงานเป็นทีม กำหนดสิทธิ์ได้รายคน',
    desc: 'เชิญลูกทีมทางอีเมล แล้วเลือกบทบาทให้แต่ละคน เช่น นายหน้าร่วมไม่เห็นเบอร์เจ้าของทรัพย์ ทีมสำรวจเห็นเฉพาะเขตที่มอบหมาย — ข้อมูลสำคัญไม่รั่วออกนอกทีม',
    icon: <IconUsers size={22} />,
  },
  {
    title: 'แผนพาลูกค้าชมทรัพย์',
    desc: 'เลือกทรัพย์ที่จะพาลูกค้าไปดู ระบบจัดเป็นเส้นทางให้ กดนำทาง Google Maps และโทรนัดเจ้าของได้จากในระบบ',
    icon: <IconRoute size={22} />,
  },
  {
    title: 'ระบบช่วยจำ ไม่มีดีลหลุด',
    desc: 'ตั้งนัดติดตามผูกกับทรัพย์ บันทึกผลไว้ทุกครั้ง ระบบเตือนเมื่อถึงวันนัด และเตือนก่อนสัญญาเช่าจะหมด',
    icon: <IconBell size={22} />,
  },
  {
    title: 'เอกสารเปรียบเทียบเสนอลูกค้า',
    desc: 'เลือกทรัพย์ 2–4 ตัว ได้ตารางเปรียบเทียบพร้อมพิมพ์เป็น PDF บันทึกเก็บไว้เป็นชอร์ตลิสต์ ใช้เสนอลูกค้ารายอื่นซ้ำได้ไม่ต้องทำใหม่',
    icon: <IconCompare size={22} />,
  },
  {
    title: 'ส่งลิงก์ให้ลูกค้าดู ราคาไม่เปลี่ยนกลางทาง',
    desc: 'ส่งลิงก์ทาง LINE ลูกค้ากดเปิดดูได้เลยไม่ต้องล็อกอิน โดยไม่เห็นเบอร์เจ้าของและพิกัด กันติดต่อข้ามหัวคุณ ราคาตรึงตามวันที่เสนอ และเห็นว่าลูกค้าเปิดดูกี่ครั้ง',
    icon: <IconLink size={22} />,
  },
  {
    title: 'Dashboard เห็นภาพทั้งพอร์ต',
    desc: 'เห็นมูลค่าพอร์ตรวม ทรัพย์ประเภทไหนเยอะ ทำเลไหนขายดี — กดที่กราฟเพื่อเปิดดูรายการได้เลย',
    icon: <IconChart size={22} />,
  },
  {
    title: 'ผู้ช่วย AI ประจำทีม',
    desc: 'ถามเป็นภาษาพูดได้เลย เช่น "มีโกดังแถวบางนาไหม" — AI ตอบจากข้อมูลจริงของทีมคุณ และช่วยสร้างแผนพาชม/นัดติดตามให้',
...
const TRUST = [
  {
    ...
    desc: 'นำเข้าไฟล์ Excel หรือ CSV เดิมได้เลย ระบบจับคู่คอลัมน์ให้อัตโนมัติ รองรับวันที่แบบ พ.ศ.',
    icon: <IconUpload size={22} />,
  },
  {
    title: 'ข้อมูลแยกองค์กร ปลอดภัยจริง',
    desc: 'แต่ละบริษัทเห็นเฉพาะข้อมูลของตัวเอง ในทีมก็กำหนดได้ว่าใครเห็นอะไร และ 1 บัญชีใช้ได้ทีละเครื่อง กันการแชร์รหัสผ่านกันใช้',
    icon: <IconShield size={22} />,
  },
  {
    title: 'ใช้บนมือถือได้เต็มรูปแบบ',
    desc: 'เปิดในเบราว์เซอร์มือถือ แล้วกด "เพิ่มไปยังหน้าจอหลัก" ใช้เหมือนแอป — ถ่ายรูปและปักหมุดจากหน้างานได้',
...
const PLANS = [
  {
    ...
    points: [
      ...
      'ชอร์ตลิสต์ + ลิงก์เสนอลูกค้า (ตรึงราคาวันที่เสนอ)',
    ],
    cta: '{trial}',
    featured: false,
  },
...
const COMPARE: { label: string; free: boolean | string; pro: boolean | string }[] = [
  ...
  { label: 'บทบาททีม 8 ระดับ — เลือกได้ว่าใครเห็นอะไร', free: true, pro: true },
  { label: 'กำหนดเขตพื้นที่ให้ทีมสำรวจ', free: true, pro: true },
  { label: '1 บัญชีใช้ได้ทีละเครื่อง (กันแชร์บัญชี)', free: true, pro: true },
  { label: 'เอกสารเปรียบเทียบ + พิมพ์ PDF', free: true, pro: true },
  // /compare ไม่ถูกล็อกตามแพ็กเกจ → บันทึกชอร์ตลิสต์และส่งลิงก์ได้ทุกแพ็กเกจ
  // (ตัวบทวิเคราะห์ AI ในเอกสารยังต้อง Pro — อยู่แถว "ผู้ช่วย AI" ด้านล่าง)
  { label: 'บันทึกชอร์ตลิสต์เสนอลูกค้า เก็บไว้ใช้ซ้ำ', free: true, pro: true },
  { label: 'ส่งลิงก์ให้ลูกค้าดูเอง ไม่ต้องล็อกอิน (ตรึงราคาวันที่เสนอ)', free: true, pro: true },
...
const STEPS = [
  ...
  { n: '3', title: 'ใช้ได้เลยทั้งทีม', desc: 'ทำงานพร้อมกันทั้งคอมและมือถือ พร้อมแผนที่ แผนพาชม ลิงก์เสนอลูกค้า และผู้ช่วย AI' },
]
