...
const FEATURES = [
  ...
  },
  {
    ...
    desc: 'โกดัง โรงงาน โชว์รูม ออฟฟิศ บ้าน คอนโด ที่ดินเปล่า — ฟอร์มปรับฟิลด์ให้ตามประเภทอัตโนมัติ (บ้านถามห้องนอน ที่ดินถาม FAR/ผังเมือง) พร้อมรูปภาพ เอกสารสิทธิ์แนบไฟล์ ค้นหา/กรองทันใจ (รวมสถานะ ว่าง/เช่าแล้ว/ขายแล้ว) และแผนที่หมุดแยกสีตามประเภท',
  ...
  },
  {
    ...
    desc: 'นัดติดตามผูกกับทรัพย์แต่ละตัว จดผลทุกครั้ง (โทรไม่รับ นัดเลื่อน เจ้าของขอคิดก่อน) เป็นประวัติย้อนดูได้ แจ้งเตือนถึงมือถือเช้าวันนัด และเตือนล่วงหน้าเมื่อสัญญาเช่าใกล้หมด (โอกาสต่อสัญญา) — ปิดดีลแล้วกด "เช่าแล้ว/ขายแล้ว" ป้ายสถานะขึ้นทุกหน้าทันที',
...
const COMPARE: { label: string; free: boolean | string; pro: boolean | string }[] = [
  ...
  { label: 'แจ้งเตือนสัญญาเช่าใกล้หมด (60/30 วัน)', free: false, pro: true },
